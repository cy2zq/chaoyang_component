...
export const reorderQuoteMap = ({ quoteMap, source, destination, columnKey }: any): any => {
  const current: Quote[] = [
    ...quoteMap?.find((item: any) => item?.[columnKey] == source.droppableId)?.list,
  ];
  const next: Quote[] = [
    ...quoteMap?.find((item: any) => item?.[columnKey] == destination.droppableId)?.list,
  ];
